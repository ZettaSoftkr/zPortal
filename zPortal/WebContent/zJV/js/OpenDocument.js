...
mindmaps.OpenDocumentView = function() {
	...
	var $openCloudButton = $("#button-open-db").button().click(function() {
		if (self.openDbClicked) {
			self.openDbClicked();
		}
	});
...
mindmaps.OpenDocumentPresenter = function(eventBus, mindmapModel, view, filePicker) {
	 ...
	 * @ignore
	 */
	view.openDbClicked = function(e) {
		
		rootNode = new MRootNode();
		//alert(JSON.stringify(rootNode));
		rootNode.getNodesWithIdAndDepth(dataSet, "0", 3);
		//alert(JSON.stringify(rootNode));

		//mindmapModel.setDocument(rootNode);
		//document.write(JSON.stringify(rootNode));
		
		url = "visualization/mindMap1.do";
		$.ajax({
