...
function qlikviewInfoSave(qlikviewId){
	...
	$.ajax({
		...
		//	$("#qlikviewUrl").attr("src","http://localhost:9090/qlikview/formLogin1.htm?userNm=" + data['bi_qlikview_user_id'] + "&userPw="+ data['bi_qlikview_user_pwd']);
...
function getMenuList(id, selectId, clearSelect) {
	...
	$.ajax({
		...
		type : "POST",
		data : {
			bi_portal_menu_parent_id : id ,bi_menu_type_yn : 'R'
		},
		datatype : 'json',
		success : function(data) {
...
function getReportSelected(selectId,val) {	

	$.ajax({
		
		url : "/"+serviceName+"/reportCycle/getQlikViewListData.do",
		type : "POST",
		datatype : 'json',		
		beforeSend : function() {
			//$(".progress").show();
		...
		success : function(data) {
			...
			$("#edit_"+selectId).empty();
			 $.each(data, function(key, value) {
					$("#edit_"+selectId).append("<option value='"+ this['parentNm']+ ">" + this['nm'] +"'>"+ this['parentNm']+ "[ " + this['nm'] + " ]</option>");
			  });
