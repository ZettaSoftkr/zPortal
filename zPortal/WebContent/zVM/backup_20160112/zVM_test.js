...
function init() {
	if (typeof (Storage) === "undefined") {
		...
		currentFile.textContent = "웹 스토리지를 지원할 수 없습니다.";
	}
	...
	// define the appearance of tooltips, shared by various templates
	var tooltiptemplateV = $(go.Adornment, go.Panel.Auto, $(go.Shape, "RoundedRectangle", {
		fill : "whitesmoke",
		stroke : "gray"
	}), $(go.TextBlock, {
		margin : 3,
	...
	function nodeActivityTaskTypeConverter(s) {
		var tasks = [ "Empty", "BpmnTaskMessage", "BpmnTaskUser", "BpmnTaskManual", // Custom
																					// hand
																					// symbol
	...
	// location of event on boundary of Activity is based on the index of the
	// event in the boundaryEventArray
	...
	var boundaryEventItemTemplate = $(go.Panel, "Spot", {
		...
		fromMaxLinks : 1,
		toMaxLinks : 0
	}, new go.Binding("portId", "portId"), new go.Binding("alignment", "alignmentIndex", nodeActivityBESpotConverter),
			$(go.Shape, "Circle", {
				...
				// nodeActivityBEFromSpotConverter, 0 & 1 go on bottom, all
				// others on top of activity
				if (s < 2)
	...
	var dataStorageNodeMenu = $(go.Adornment, "Vertical", $("ContextMenuButton", $(go.TextBlock, "  편    집     ", {
		margin : 3
	}), {
		click : function(e, obj) {
	...
	var dataObjectNodeMenu = $(go.Adornment, "Vertical", $("ContextMenuButton", $(go.TextBlock, "  편    집     ", {
		margin : 3
	}), {
		click : function(e, obj) {
	...
	function makeMarkerPanel(sub, scale) {
		return $(go.Panel, "Horizontal", {
			...
			strokeWidth : ActivityMarkerStrokeWidth,
			fill : null
		}, new go.Binding("visible", "isCompensation")), makeSubButton(sub)); // end
																				// activity
																				// markers
																				// horizontal
																				// panel
	...
	// move a selected part into the Foreground layer, so it isn"t obscured by
	// any non-selected parts
	...
	}, new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify), $(go.Panel, "Spot", $(
												...
												// Service, etc via converter
	{
		...
		width : 22,
		height : 22
	}, new go.Binding("fill", "taskType", nodeActivityTaskTypeColorConverter), new go.Binding("figure", "taskType",
			nodeActivityTaskTypeConverter)), // end Task Icon
	makeMarkerPanel(false, 1) // sub-process, loop, parallel, sequential, ad
								// doc and compensation markers
	...
	var visualizationNodeTemplate = $(go.Node, "Spot", {
		locationObjectName : "SHAPE",
		locationSpot : go.Spot.Center,
		resizable : true,
		resizeObjectName : "PANEL",
	...
	$(go.TextBlock, // the center text
	{
		...
		margin : 12,
		editable : true
	}, new go.Binding("text").makeTwoWay()), $(go.Picture, {
		row : 0,
		column : 1,
	...
	}, new go.Binding("source", "chartType", theChartConverter))) // end Auto
																	// Panel
	); // end go.Node, which is a Spot Panel with bound itemArray

	// ------------------------------- template for Activity / Task node in
	// Palette -------------------------------
	...
	// task icon
	$(go.Shape, "BpmnTaskScript", // will be None, Script, Manual, Service,
									// etc via converter
	{
		alignment : new go.Spot(0, 0, 5, 5),
		alignmentFocus : go.Spot.TopLeft,
		...
		height : 22 / palscale
			...
			nodeActivityTaskTypeConverter)), makeMarkerPanel(false, palscale) 
	), // End Spot panel
	$(go.TextBlock, // the center text
	{
		alignment : go.Spot.Center,
											...
											// rectangle
	{
		...
		fill : null,
		visible : false
	}, new go.Binding("visible", "isTransaction")), makeMarkerPanel(true, palscale) 
	), // end main body rectangles spot panel
	$(go.TextBlock, // the center text
	{
		alignment : go.Spot.Center,
		textAlign : "center",
		margin : 2
	}, new go.Binding("text"))); // end go.Group
	...
	}, $(go.Panel, "Spot", $(go.Shape, "Circle", // Outer circle
	{
		strokeWidth : 1,
		name : "SHAPE",
		desiredSize : new go.Size(EventNodeSize, EventNodeSize),
	...
	}), new go.Binding("stroke", "eventDimension", nodeEventDimensionStrokeColorConverter), new go.Binding(
			"strokeDashArray", "eventDimension", function(s) {
				return (s === 3 || s === 6) ? [ 4, 2 ] : null;
			}), new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify)), // end
																										// main
																										// shape
	...
	new go.Binding("visible", "eventDimension", function(s) {
		...
		desiredSize : new go.Size(EventNodeSymbolSize, EventNodeSymbolSize),
		stroke : "black"
	}, new go.Binding("figure", "eventType", nodeEventTypeConverter), new go.Binding("fill", "eventDimension",
			nodeEventDimensionSymbolFillConverter))), // end Auto Panel
	$(go.TextBlock, {
		alignment : go.Spot.Center,
		textAlign : "center",
		margin : 5,
	...
	function nodeGatewaySymbolTypeConverter(s) {
		var tasks = [ "NotAllowed", "ThinCross", // 1 - Parallel
		"Circle", // 2 - Inclusive
	...
	// can be resided according to the user's desires
	{
		resizable : false,
		resizeObjectName : "SHAPE"
	}, $(go.Panel, "Spot", $(go.Shape, "Diamond", {
		strokeWidth : 1,
	...
	// new go.Binding("visible", "gatewayType", function(s) { return s !== 4;
	// }), // comment out if you want exclusive gateway to be X instead of
	// blank.
	...
	$(go.Shape, "Circle", // Inner circle
	{
		...
		margin : 5,
		editable : true
	}, new go.Binding("text").makeTwoWay())); // end go.Node Vertical
	...
	// new go.Binding("desiredSize", "gatewayType", new go.Size(EventNodeSize/2,
	// EventNodeSize/2)),
	new go.Binding("visible", "gatewayType", function(s) {
		return s >= 5;
	}) // only visible for > 5
	...
	var annotationNodeTemplate = $(go.Node, "Auto", {
		background : GradientLightGray,
		locationSpot : go.Spot.Center
	}, new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, "Annotation", 
	{
		portId : "",
		fromLinkable : true,
		cursor : "pointer",
	...
	var dataObjectNodeTemplate = $(go.Node, "Vertical", {
		locationObjectName : "SHAPE",
		locationSpot : go.Spot.Center,
		contextMenu : dataObjectNodeMenu
	}, new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, "File", {
		name : "SHAPE",
		portId : "",
		fromLinkable : true,
		toLinkable : true,
	...
	var dataStoreNodeTemplate = $(go.Node, "Vertical", {
		locationObjectName : "SHAPE",
		locationSpot : go.Spot.Center,
		contextMenu : dataStorageNodeMenu
	}, new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, "Database", {
		name : "SHAPE",
		portId : "",
		fromLinkable : true,
		toLinkable : true,
	...
	// ------------------------------------------ private process Node Template
	// Map ----------------------------------------------

	var privateProcessNodeTemplate = $(go.Node, "Auto", {
	...
	}), $(go.Panel, "Table", // table with 2 cells to hold header and lane
	{
		...
		editable : true,
		textAlign : "center"
	}, new go.Binding("text").makeTwoWay()), $(go.RowColumnDefinition, {
		column : 1,
	...
	var privateProcessNodeTemplateForPalette = $(go.Node, "Vertical", {
		locationSpot : go.Spot.Center
	}, $(go.Shape, "Process", {
		fill : DataFill,
	...
	var layerGroupTemplateForPalette = $(go.Group, "Auto");
	
	var layoutTemplateForPalette = $(go.Group, "Vertical", {
		locationSpot : go.Spot.Center,
		isSubGraphExpanded : false
	}, $(go.Picture, {
		...
		toLinkable : true,
		cursor : "pointer"
	}, new go.Binding("source", "key", convertCategoryImage)), $(go.TextBlock, "사용자", textStyle(), new go.Binding(
			"text")));
	...
	// move a selected part into the Foreground layer, so it isn't obscured by
	// any non-selected parts
	// new go.Binding("layerName", "isSelected", function (s) { return s ?
	// "Foreground" : ""; }).ofObject(),
	$(go.Panel, "Auto", $(go.Shape, "RoundedRectangle", {
		...
		fromSpot : go.Spot.RightSide,
		toSpot : go.Spot.LeftSide
	}, new go.Binding("desiredSize", "size", go.Size.parse).makeTwoWay(go.Size.stringify), new go.Binding(
			"strokeWidth", "isCall", function(s) {
				return s ? ActivityNodeStrokeWidthIsCall : ActivityNodeStrokeWidth;
			})), $(go.Panel, "Vertical", {
	...
	}, $(go.TextBlock, // label
	{
		margin : 3,
		editable : true
	}, new go.Binding("text", "text").makeTwoWay(), new go.Binding("alignment", "isSubGraphExpanded", function(s) {
		return s ? go.Spot.TopLeft : go.Spot.Center;
	})),
	// create a placeholder to represent the area where the contents of the
	// group are
	$(go.Placeholder, {
	...
	var layerGroupTemplate = $(go.Group, 
		{ 
		...
		var layoutGroupTemplate = $(go.Group, "Auto", 
			groupStyle(), 
			{ 
				layout : $(PoolLayout, {
	...
	nodeTemplateMap.add("activity", visualizationNodeTemplate);
	...
	nodeTemplateMap.add("visualization", visualizationNodeTemplate);
	// for the default category, "", use the same template that Diagrams use by
	// default
	// this just shows the key value as a simple TextBlock

	var groupTemplateMap = new go.Map("string", go.Group);
	groupTemplateMap.add("Layout", layoutGroupTemplate);
	groupTemplateMap.add("Layer", layerGroupTemplate);

	// create the nodeTemplateMap, holding special palette "mini" node
	// templates:
	var palNodeTemplateMap = new go.Map("string", go.Node);
	palNodeTemplateMap.add("activity", visualizationNodeTemplateForPalette);
	...
	palNodeTemplateMap.add("visualization", visualizationNodeTemplateForPalette);

	var palGroupTemplateMap = new go.Map("string", go.Group);
	palGroupTemplateMap.add("Layout", layoutTemplateForPalette);
	palGroupTemplateMap.add("Layer", layerGroupTemplateForPalette);
	...
	jQuery.ajax({
		...
		success : function(msg) {
			if (window.JSON && window.JSON.parse) {
				try {
					objectTypes = window.JSON.parse(msg);
					console.info("objectTypes : " + JSON.stringify(objectTypes));
					var sObjects = jQuery.grep(objectTypes.objectTypeInfos, function(entry) {
						return (entry.leftGroup === 'system');
					});
					var vObjects = jQuery.grep(objectTypes.objectTypeInfos, function(entry) {
						return (entry.leftGroup === 'visualization');
					});
					var dObjects = jQuery.grep(objectTypes.objectTypeInfos, function(entry) {
						return (entry.leftGroup === 'dataProcess');
					});
					console.info("dObjects : " + JSON.stringify(dObjects));
					myPaletteLevel1.model = $(go.GraphLinksModel, {
						copiesArrays : true,
						copiesArrayObjects : true,
						nodeDataArray : dObjects
					}); // end model
					
					console.info("vObjects : " + JSON.stringify(vObjects));
					myPaletteLevel3.model = $(go.GraphLinksModel, {
						copiesArrays : true,
						copiesArrayObjects : true,
						nodeDataArray : vObjects
					}); // end model
						...
						success : function(msg) {
							if (window.JSON && window.JSON.parse) {
								try {
									objectTypes = window.JSON.parse(msg);
									var list = new Array();
									for (var index in objectTypes){
										for(var sub in objectTypes[index]){
											list.push(objectTypes[index][sub])
										}
									}
									console.info("layouts : " + JSON.stringify(objectTypes));
									jQuery.merge(sObjects, list);						
									
									console.info("sObjects : " + JSON.stringify(sObjects));
									myPaletteLevel2.model = $(go.GraphLinksModel, {
										copiesArrays : true,
										copiesArrayObjects : true,
										nodeDataArray : sObjects
									}); 
									
								} catch (d) {
									alert("system에서 VObjectInfo 데이터를 로드할수 없습니다. error:" + d);
								}
							}
						},
...
function loadFileinit(fileName) {
	if (fileName != '_new') {
...
// this is called after nodes have been moved or lanes resized, to layout all of
// the Pool Groups again
function relayoutDiagram() {
	myDiagram.layout.invalidateLayout();
	myDiagram.findTopLevelGroups().each(function(g) {
		if (g.category === "Layout")
			g.layout.invalidateLayout();
	});
	myDiagram.layoutDiagram();
}

// compute the minimum size of a Pool Group needed to hold all of the Lane
// Groups
function computeMinPoolSize(pool) {
	// assert(pool instanceof go.Group && pool.category === "Layout");
	var len = MINLENGTH;
...
function computeLaneSize(lane) {
	// assert(lane instanceof go.Group && lane.category !== "Layout");
...
LaneResizingTool.prototype.canStart = function() {
	...
	// if this is a resize handle for a "Layer", we can start.
	var diagram = this.diagram;
	if (diagram === null)
		return;
	var handl = this.findToolHandleAt(diagram.firstInput.documentPoint, this.name);
	if (handl === null || handl.part === null || handl.part.adornedObject === null
			|| handl.part.adornedObject.part === null)
		return false;
	return (handl.part.adornedObject.part.category === "Layer");
}
...
LaneResizingTool.prototype.resize = function(newr) {
	...
	relayoutDiagram(); // now that the lane has changed size, layout the pool
						// again
};
// end LaneResizingTool class

// define a custom grid layout that makes sure the length of each lane is the
// same
...
function PoolLayout() {
	...
	// This is useful when Groups can be moved up and down in order to change
	// their order.
...
PoolLayout.prototype.doLayout = function(coll) {
	...
	var pool = this.group;
	if (pool !== null && pool.category === "Layout") {
		// make sure all of the Group Shapes are big enough
		var minsize = computeMinPoolSize(pool);
		pool.memberParts.each(function(lane) {
			if (!(lane instanceof go.Group))
				return;
			if (lane.category !== "Layout") {
				var shape = lane.resizeObject;
				if (shape !== null) {
					shape.size = lane.data.size;
				}
			}
		});
	}
	// now do all of the usual stuff, according to whatever properties have been
	// set on this GridLayout
...
function askSpace() {
	var space = prompt("노드사이의 간격 (in pixels):", "0");
	return space;
}

var UnsavedFileName = "(저장되지 않은 파일)";
...
function newDocument() {
	...
	if (myDiagram.isModified) {
		var save = confirm(getCurrentFileName() + " 파일을 변경하시겠습니끼?");
		if (save) {
			saveDocument();
		}
	}
...
function saveDocumentAs() {
	if (checkLocalStorage()) {
		var saveName = prompt("이름을 변경합니다.", getCurrentFileName());
...
function convertCategoryImage(category) {
	return "/zPortal/zVM/images/objects/" + category.toLowerCase() + ".png";
}
