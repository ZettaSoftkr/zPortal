...
function init() {
    ...
    var dataObjectNodeMenu = $(go.Adornment, "Vertical",
        $("ContextMenuButton", $(go.TextBlock, "  편    집     ", {
            margin: 3
        }), {
            click: function(e, obj) {
    ...
    var activityNodeMenu = $(go.Adornment, "Vertical",
        $("ContextMenuButton", $(go.TextBlock, "  실행화면 보기     ", {
            margin: 3
        }), {
    ...
    var visualizationNodeTemplate = $(go.Node, "Spot", {
        ...
        new go.Binding("layerName", "isSelected", function(s) {
            ...
            }, new go.Binding("source", "chartType", theChartConverter))) // end Auto Panel
    ); // end go.Node, which is a Spot Panel with bound itemArray
            ...
            $(go.Shape, "BpmnTaskScript", // will be None, Script, Manual, Service, etc via converter
                {
                    ...
                    nodeActivityTaskTypeConverter)), makeMarkerPanel(false, palscale) // sub-process,  loop, parallel, sequential, ad doc and compensation markers
        ), // End Spot panel
        $(go.TextBlock, // the center text
            {
                alignment: go.Spot.Center,
    ...
    var visualizationNodeTemplateForPalette = $(go.Node, "Vertical", {
            ...
            selectionAdorned: false
                ...
                })), $(go.Shape, "RoundedRectangle", // the inner "Transaction" rounded rectangle
                {
                    ...
                    fill: null,
                    visible: false
                }, new go.Binding("visible", "isTransaction")), makeMarkerPanel(false, palscale) // sub-process,  loop, parallel, sequential, ad doc and compensation markers
        ), // End Spot panel
        $(go.Picture, {
            row: 0,
            column: 1,
            ...
            imageStretch: go.GraphObject.Uniform,
            alignment: go.Spot.TopRight
        }, new go.Binding("source", "chartType", theChartConverter))); // End Node
        ...
        // can be resided according to the user's desires
        {
            ...
            resizeObjectName: "SHAPE"
                ...
                new go.Binding("fill", "eventDimension", function(s) {
                    ...
                    "strokeDashArray", "eventDimension",
                    function(s) {
    ...
    var annotationNodeTemplate = $(go.Node, "Auto", {
        ...
        locationSpot: go.Spot.Center
    }, new go.Binding("location", "loc", go.Point.parse).makeTwoWay(go.Point.stringify), $(go.Shape, "Annotation", // A left bracket shape
        {
            portId: "",
            fromLinkable: true,
            cursor: "pointer",
    ...
    var laneEventMenu = // context menu for each lane
        $(go.Adornment, "Vertical", $("ContextMenuButton", $(go.TextBlock, "패널 추가"),
            // in the click event handler, the obj.part is the Adornment; its adornedObject is the port
            {
                click: function(e, obj) {
    ...
    var poolEventMenu = // context menu for each lane
        $(go.Adornment, "Vertical", $("ContextMenuButton", $(go.TextBlock, " 실행화면보기 "),
            // in the click event handler, the obj.part is the Adornment; its adornedObject is the port
            {
                click: function(e, obj) {
                	 window.open("/zPortal/zVM/zViewer.html?fileName=" + getCurrentFileName() + "&objectKey=" + obj.part.data.key);
                }
            }));
    
    // Add a lane to pool (lane parameter is lane above new lane)
    function addLaneEvent(lane) {
        myDiagram.startTransaction("패널추가");
        ...
        myDiagram.commitTransaction("패널추가");
    }
    ...
    var swimLanesGroupTemplate = $(go.Group, "Spot", groupStyle(), {
        ...
        $(go.Shape, "Rectangle", // this is the resized object
            ...
            new go.Binding("fill", "color"), new go.Binding("desiredSize", "size", go.Size.parse)
        ...
        $(go.Panel, "Horizontal", {
            ...
            $(go.TextBlock, // the lane label
                ...
                }, new go.Binding("visible", "isSubGraphExpanded").ofObject(), new go.Binding("text", "text")
                .makeTwoWay()), $("SubGraphExpanderButton", {
                margin: 4,
                angle: -270
        ...
        $(go.Placeholder, {
        ...
        }, $(go.TextBlock, // this TextBlock is only seen when the swimlane is collapsed
            {
                ...
                return !e;
            }).ofObject(), new go.Binding("text", "text").makeTwoWay()))); // end swimLanesGroupTemplate
    ...
    var poolGroupTemplate = $(go.Group, "Auto", groupStyle(), { // use a simple layout that ignores links to stack the "lane" Groups on top of each other
    	contextMenu: poolEventMenu,
        layout: $(PoolLayout, {
    ...
    var myPaletteLevel3 = $(go.Palette, "myPaletteLevel3", { // share the templates with the main Diagram
        nodeTemplateMap: palNodeTemplateMap,
        groupTemplateMap: palGroupTemplateMap,
        layout: $(go.GridLayout, {
            cellSize: new go.Size(1, 1),
    ...
    // initialize the third Palette, random other stuff
    var myPaletteLevel4 = $(go.Palette, "myPaletteLevel4", { // share the templates with the main Diagram
        nodeTemplateMap: palNodeTemplateMap,
        groupTemplateMap: palGroupTemplateMap,
        layout: $(go.GridLayout, {
            cellSize: new go.Size(1, 1),
			...
			if (window.JSON && window.JSON.parse) {
				try {
					...
					var sObjects = jQuery.grep(objectTypes.objectTypeInfos, function(entry) {
								  return ( entry.leftGroup === 'system' || entry.leftGroup === 'layout' );
							});
					...
					console.info("sObjects : " + JSON.stringify(sObjects));
					 ...
					 myPaletteLevel4.model = $(go.GraphLinksModel, {
				        copiesArrays: true,
				        copiesArrayObjects: true,
...
function loadFileinit(fileName) {
	if(fileName != '_new'){
        ...
        jQuery.ajax({
			...
			data : {fileName: fileName, objectKey:'main'}
		});
