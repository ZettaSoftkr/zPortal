...
mindmaps.CanvasView = function() {
	 ...
	 * 백그라운드 셋팅
	 */
	this.setBackGroundImg = function(path) {
		var drawingArea = this.$getDrawingArea();
		drawingArea.css("background-image", 'url(' + path + ')');
		//drawingArea.css("image-rendering", "auto");
	};
		...
		if (!node.isRoot()) {
			...
			$node.one("mouseenter", function() {
				$node.draggable({
					...
					stop : function(e, ui) {
						nodeDragging = false;
						var pos = new mindmaps.Point(ui.position.left / self.zoomFactor, ui.position.top
								/ self.zoomFactor);
	...
	function Creator(view) {
		...
		$wrapper.draggable({
			...
			drag : function(e, ui) {
				...
				drawLineCanvas($canvas, self.depth + 1, offsetX, offsetY, $fakeNode, $node, self.lineColor);
			},
			stop : function(e, ui) {
				dragging = false;
				...
				if (self.dragStopped) {
					...
					var distance = mindmaps.Util.distance($wpLeft - nubLeft, $wpTop - nubTop);
