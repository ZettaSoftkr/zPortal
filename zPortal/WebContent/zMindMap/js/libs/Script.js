...
function loadThem() {
	Qva.AddExtension('wordCloud', function() {
		...
		function draw(words) {
			d3.select("#" + divName).append("svg").attr("margin", "0px").attr("padding", "0px").attr("width", _this.GetWidth()).attr("height", _this.GetHeight()).append("g").attr("transform", "translate(" + (_this.GetWidth() / 2) + "," + (_this.GetHeight() / 2) + ")").selectAll("text").data(words).enter().append("text").style("font-size", function(d) {
				return d.size + "px";
